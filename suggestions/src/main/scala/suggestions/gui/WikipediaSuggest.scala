...
/*
      ...
      searchTerms.map[Observable[Try[List[String]]]](v =>
        wikiSuggestResponseStream(v).concat)
    }
//      Observable.from(wikipediaSuggestion(v)).timedOut(3).recovered).concat
...
*/
    ...
    val selections: Observable[String] = {
      button.clicks.filter(_ => {
        ...
        Observable.from(suggestionList.selection.items.toList)
      })
    }

    // TO IMPLEMENT
    val pages: Observable[Try[String]] = {
      selections.concatRecovered(x => {
        wikiPageResponseStream(x)
      })
    }

/*
      selections.map(url => {
        Observable.from(wikipediaPage(url)).timedOut(3).recovered}).concat
    }
*/
