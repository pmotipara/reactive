  ...
  implicit class TextFieldOps(field: TextField) {
    ...
    def textValues: Observable[String] = {
      Observable.create[String]( obs => {
        field.subscribe(new Reactions.Reaction {
          def apply(evt: Event) = {
            obs.onNext(field.text)
          }
          def isDefinedAt(event: Event) = event.isInstanceOf[ValueChanged]
  ...
  implicit class ButtonOps(button: Button) {
    ...
    def clicks: Observable[Button] = {
      Observable.create[Button](obs => {
        button.subscribe(new Reactions.Reaction {
          def apply(event: Event) = {
            obs.onNext(button)
          }

          def isDefinedAt(event: Event) = event.isInstanceOf[ButtonClicked]
        })
        Subscription({obs.onCompleted})
      })
    }
