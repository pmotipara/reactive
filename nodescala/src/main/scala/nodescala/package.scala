...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
     ...
     */
    def all[T](fs: List[Future[T]]): Future[List[T]] = Future.fold(fs)(List[T]())((r,t)=>(r :+ t))
     ...
     */
    def any[T](fs: List[Future[T]]): Future[T] = Future.firstCompletedOf(fs)// Future.reduce(fs)((z,a) => z)
 ...
 /*     val timer = new Timer()
      ...
      val f = Future(timer.schedule(new TimerTask {
        def run(): Unit = {}
      }, t.toMillis))

      p.completeWith(f)
      p.future*/
      //Future (blocking(Await.ready(Future.never, t)))
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
     ...
     */
    def now: T = if (f.isCompleted) Await.result(f, 0 millis) else throw new NoSuchElementException
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      Future(cont({Await.ready(f, Duration.Inf)}))
    }
