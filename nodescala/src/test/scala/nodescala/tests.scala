...
class NodeScalaSuite extends FunSuite with AsyncAssertions {
  ...
  test("any complets with first completed future value!") {
    val f = Future.any(List(Future { 1 }, Future { 2 }, Future { throw new Exception }))
    assert(Await.result(f, 0 nanos) == 1)

  }

  test("any fails with exception when first completed future is an exception") {
    val f = Future.any(List(Future { throw new Exception }, Future { 2 }, Future { throw new Exception }))
